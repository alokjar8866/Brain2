...
export function EditContentModal({ open, onClose, contentId, initialTitle, initialTags }: EditContentModalProps) {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: async () => {
            const response = await axios.put(
                `${BACKEND_URL}/api/v1/updateContent/${contentId}`,
                { title, tags },
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input
                                ...
                                className="w-full bg-white/4 border border-white/8 rounded-xl px-3 py-2.5 text-sm text-white outline-none focus:border-[#3088fc]/50 transition-all placeholder:text-white/20"
                            />
                        </div>
